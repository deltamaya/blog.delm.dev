...
export async function load({ params }) {
	...
	renderer.code = ({ text, lang }) => {
		...
		return `<div class="relative group">
				...
				<button class="copy-button opacity-0 absolute group-hover:opacity-100 top-2 right-2 bg-neutral-900 border-[1px] border-neutral-100 dark:border-neutral-800 text-white text-sm px-2 py-1 transition-opacity duration-200">Copy</button>
				</div>`;
	};
